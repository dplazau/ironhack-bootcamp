...
select min(payments) from loan;
select loan_id from loan
where payments = 9910; -- 6312 is the client_id from the minimum payment
...
select district_id, count(client_id) as number_of_clients from `client`
...
select date, count(account_id) from loan
where date < 930907
group by date
order by date desc;
...
-- Your output should have the account_id, the type and the sum of amount, named as total_amount. Sort alphabetically by ty
...
select account_id,
round(sum(case when type='PRIJEM' then amount end), 0) as INCOMING,
round(sum(case when type='VYDAJ' then amount end), 0) as OUTGOING,
round(sum(case when type='PRIJEM' then amount end) - sum(case when type='VYDAJ' then amount end), 0) as DIFFERENCE
from trans
where account_id = 396
group by account_id;
